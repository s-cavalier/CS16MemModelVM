...
HOOK_TEMPLATE(printInstr) {
    const Word& pc = machine.readProgramCounter();
    DBG_OUT << "Reading instruction 0x"<< std::hex << std::setw(8) << std::setfill('0') << machine.readMemory().getWord(pc) << " at program counter 0x" << std::setw(8) << std::setfill('0') << pc << std::dec << '\n';
}

HOOK_TEMPLATE(printRegs) {
    DBG_OUT << "REGISTERS:\n";
    for (int i = 0; i < 32; ++i) DBG_OUT << '$' << Binary::regToString[i] << ":" << machine.readRegister(i) << ' ';
    DBG_OUT << '\n';
}

HOOK_TEMPLATE(printFPRegs) {
    DBG_OUT << "FP REGISTERS:\n";
    for (int i = 0; i < 32; ++i) DBG_OUT << "$f" << i << ":" << machine.readFPRegister(i) << ' ';
    DBG_OUT << '\n';
}
