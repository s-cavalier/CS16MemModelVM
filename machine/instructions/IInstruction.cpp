...
IMemoryInstruction::IMemoryInstruction(I_MEM_INSTR_ARGS) : IGenericInstruction(rt, rs, imm), mem(mem), tlb(tlb), asidReg(asidReg) {}
...
#define I_MEM_CONSTRCTR_INIT(x) x::x(I_MEM_INSTR_ARGS) : IMemoryInstruction(rt, rs, imm, mem, tlb, asidReg) {} void x::run()
I_MEM_CONSTRCTR_INIT(LoadByte) { rt = char(mem.getByte(Word(rs) + int(imm), tlb, asidReg & 0xFF)); }
I_MEM_CONSTRCTR_INIT(LoadByteUnsigned) { rt = Word(mem.getByte(Word(rs) + int(imm), tlb, asidReg & 0xFF)); }
I_MEM_CONSTRCTR_INIT(LoadHalfword) { rt = short(mem.getHalfWord(Word(rs) + int(imm), tlb, asidReg & 0xFF)); }
I_MEM_CONSTRCTR_INIT(LoadHalfwordUnsigned) { rt = Word(mem.getHalfWord(Word(rs) + int(imm), tlb, asidReg & 0xFF)); }
I_MEM_CONSTRCTR_INIT(LoadWord) { rt = mem.getWord(Word(rs) + int(imm), tlb, asidReg & 0xFF); }
I_MEM_CONSTRCTR_INIT(StoreByte) { mem.setByte(Word(rs) + int(imm), Byte(rt), tlb, asidReg & 0xFF); }
I_MEM_CONSTRCTR_INIT(StoreHalfword) { mem.setHalfWord(Word(rs) + int(imm), HalfWord(rt), tlb, asidReg & 0xFF); }
I_MEM_CONSTRCTR_INIT(StoreWord) { mem.setWord(Word(rs) + int(imm), rt, tlb, asidReg & 0xFF); }
